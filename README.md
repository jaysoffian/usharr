...
The Plex, Radarr, and Sonarr integrations require Usharr to match local paths against the paths reported to it by the integrations; if these paths are not the same for any given integration, you must configure `path_map` for that integration. e.g.
